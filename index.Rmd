...
```{r tameimpala1, echo=FALSE}
p <- ggplot(currentsFeatures, aes(x = tempo, y = energy, size = valence, color = loudness)) + geom_point() + ggtitle("Currents")
ggplotly(p)
...
p <- ggplot(elecLadylandFeatures, aes(x = tempo, y = energy, size = valence, color = loudness)) + geom_point() + ggtitle("Electric Ladyland")
ggplotly(p)
```
...
### "The Less I Know The Better" {data-height=600}
...
### "The Burning of the Midnight Lamp" {data-height=600}
...
ti <-
  get_playlist_audio_features("tameimpala", "5VK2XNDMxUDZXG081cNPFL") |>
  add_audio_analysis() |>
  mutate(
